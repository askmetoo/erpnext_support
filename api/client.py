...
def update_erpnext_support_issue_status_and_communications(erpnext_support_issues, response):
	...
	for erpnext_support_issue in erpnext_support_issues:
		...
		last_sync_on = response.get(erpnext_support_issue.name).get("last_sync_on")
		...
		if last_sync_on:
			frappe.db.set_value("ERPNext Support Issue", erpnext_support_issue.name, "last_sync_on", get_datetime(last_sync_on))
...
def create_communications_for_sync(erpnext_support_issue, subject, description, raised_by, recipients, attachments=None):
...
def change_support_issue_status(status, frappe_issue_id):
	params = {
		"status": status,
