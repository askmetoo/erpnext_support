...
from frappe.utils import add_days, today, get_datetime, getdate, date_diff, get_datetime_str
from frappe.installer import update_site_config
from frappe.utils.file_manager import save_file
from frappe.utils import now_datetime
from erpnext_support.api.server_utils import get_attachments
...
def send_notification_email_to_assignees(issue):
	...
	frappe.sendmail(
		subject=_("Issue {0} closed by Customer.").format(frappe.bold(issue.name)),
...
def reset_client_issue_id(doc=None, method=None):
	if not doc.get("issue_split_from"):
		return
